...
const STATUS_THEMES = {
    ...
    unclear: 'bg-slate-500/10 text-slate-400 border-slate-500/20',
...
const LeadRow = ({ lead, onUpdate, onViewMessages, onEdit, onDelete, onOpenOutreachStatus, onGenerateProposal }) => {
    ...
    return (
        <tr className="hover:bg-white/5 transition-colors group">
            ...
            {/* Outreach State Cell */}
            <td className="px-6 py-4 whitespace-nowrap border-r border-white/5 last:border-0 min-w-[120px]">
                <button
                    onClick={() => onOpenOutreachStatus(lead)}
                    className={`px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest border transition-all duration-300 hover:scale-105 active:scale-95 flex items-center gap-1.5
                        ${isOutreachSent
                    ...
                    `}
                    title="Click to view detailed Outreach Logs"
                >
                    <span>{isOutreachSent ? 'Sent' : isOutreachQueued ? 'Queued' : 'Not Sent'}</span>
            ...
            <td className="px-6 py-4 whitespace-nowrap text-right flex items-center justify-end gap-2">
                ...
                <button
                    ...
                    title="View Outreach Logs & Status"
                >
                    <Icons.Logs />
                </button>
