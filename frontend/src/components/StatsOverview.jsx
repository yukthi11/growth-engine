...
const StatsOverview = ({ stats }) => {
    return (
        <div className="space-y-6 animate-in select-none">
            <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
                ...
                <StatCard
                    title="Active Pulse"
                    value={stats.activeSequences?.toLocaleString() || 0}
                    icon={Icons.Play}
                ...
                <StatCard
                    title="AI Interest"
                    value={stats.interestedReplies?.toLocaleString() || 0}
                    icon={Icons.Star}
            ...
            {stats.recent_keywords && stats.recent_keywords.length > 0 && (
                <div className="rounded-[32px] border border-white/5 bg-white/[0.02] p-6 premium-shadow flex items-center gap-4 flex-wrap">
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 mr-2 flex items-center gap-2">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                        Recent Targets:
                    </span>
                    {stats.recent_keywords.map((keyword, idx) => (
                        <div key={idx} className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-xs font-bold text-slate-300">
                            {keyword}
                        </div>
                    ))}
                </div>
